      ...
      if (decoded?.exp < currentTime) {
        try {
          ...
          localStorage.setItem(
            "acces_token",
            JSON.stringify(data?.acces_token)
          );
        ...
        <Router>
          <Routes>
            {routes.map((route, index) => {
              ...
              const isAuthorized =
                !isPrivateRoute || (isPrivateRoute && user?.isAdmin);

              if (!isAuthorized) {
                // nếu isAthuthorized là false nghĩa là không có quyền truy cập
                // Nếu không có private, bỏ qua route này
                return null;
              }
              const Layout =
                route.isShowHeader !== "false" ? DefaultCommponent : Fragment;
              return (
                <Route
                  key={index}
                  path={route.path}
